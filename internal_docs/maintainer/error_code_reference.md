...
For the mixed block case, the 2026-06-12 root-cause check found a client layout
bug rather than an unexplained PLC behavior. Current clients use the corrected
Write Block layout and do not perform automatic split retry. If a caller
intentionally wants two requests, use `split_mixed_blocks=True`; otherwise keep
the original PLC end code visible.
